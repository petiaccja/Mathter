...
namespace impl {
	...
	class PerspectiveBuilder {
		static_assert(!std::is_integral_v<T>, "Perspective transformations require real numbers.");
		static_assert(!is_complex_v<T>, "Perspective transformations cannot be done in the complex domain.");
	...
	private:
		...
		void Set(Matrix<U, Rows, Columns, Order, Layout, MPacked>& m) const {
			...
			const auto N = U(nearPlane);
			const auto F = U(farPlane);
			const auto n = U(projNearPlane);
			const auto f = U(projFarPlane);
			auto C = U(nearPlane) < U(0) ? U(-1) : U(1);
			const auto A = C * (f * F - n * N) / (F - N);
			auto B = C * F * N * (n - f) / (F - N);
			auto adjRatios = U(ratios(0)) / UVec(ratios);
			const auto w = std::tan(U(0.5) * absFovX);
			adjRatios /= w;
...
auto Perspective(T fovX, const Vector<T, DimMinus1, Packed>& ratios, T nearPlane, T farPlane, T projNearPlane, T projFarPlane) {
	...
	return impl::PerspectiveBuilder<NonIntegral, DimMinus1 + 1, Packed>{ fovX, ratios, nearPlane, farPlane, projNearPlane, projFarPlane };
}
...
/// <param name="fovX"/> Field of view. </param>
/// <param name="aspectRatio"> FovX/FovY, so 1.777 for a 16:9 screen. </param>
/// <param name="nearPlane"/> Lower bound of the volume on the Z axis. </param>
/// <param name="farPlane"/> Upper bound of the volume on the Z axis. </param>
/// <param name="projNearPlane"/> Near plane is taken here after projection. </param>
/// <param name="projFarPlane"/> Far plane is taken here after projection. </param>
/// <remarks> The pre-projection near and far planes must be on the same side of the last axis, i.e. both negative or both positive.
///		The post-projection near and far planes can be arbitrary, that is, near larger, near smaller, both positive, both negative,
...
template <class T>
auto Perspective(T fovX, T aspectRatio, T nearPlane, T farPlane, T projNearPlane, T projFarPlane) {
	const auto ratioX = fovX < T(0) ? T(-1) : T(1);
	const auto ratioY = T(1) / aspectRatio;
	const auto absFovX = std::abs(fovX);
	return Perspective(absFovX, Vector<T, 2, false>{ ratioX, ratioY }, nearPlane, farPlane, projNearPlane, projFarPlane);
}
